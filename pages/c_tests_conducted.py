...
df_column_rm = pd.DataFrame({'Location':pd.Series(dtype='str'),
                        'Tests Conducted-Yesterday Count': pd.Series(dtype='int'),
                        'Tests Conducted-Cumulative Count': pd.Series(dtype='int')})
...
def layout():
    ...
    html.Div([
    html.Div(dcc.DatePickerRange(id='my_date_picker_range_tc',display_format='D-M-Y',min_date_allowed=date(2022, 2, 3),max_date_allowed=obj_datepicker_tc.current_date(),initial_visible_month=date(2022, 2, 3),start_date=date(2022, 2, 3),end_date=obj_datepicker_tc.current_date(),style={"position":"relative","display":"inline-block","width":"110%","border-radius":"39px","margin-left":"115%","margin-top":"5%","margin-bottom":"7%"}),style={}),
    ...
    html.A([html.I(className="fa fa-refresh",style={})],href=dash.page_registry['pages.c_tests_conducted']['path'],style={"width":"4%","height":"2.3rem","margin-left":"0.8rem","margin-top":"1.5%","text-align":"center","font-size":"22px","background-color":"rgb(215,215,215)","border-radius":"8px","padding-top":"0.3rem","border":"2px double black"})
...
@app.callback(Output('table_tc','data'),Output('c_tc_error','children'),
    inputs=dict(start_date=Input('my_date_picker_range_tc','start_date'),end_date=Input('my_date_picker_range_tc','end_date')))
def update_tc_selected(start_date,end_date):
    ...
    if start>end:
        ...
        #         tc_n1.append(0)
        #     df_tc['Tests Conducted-Yesterday Count']=tc_n1
        ...
        #     df_tc['Tests Conducted-Yesterday Count']=df_tc_main_impo[0]
        # else:
        #     print("I am in else loop-DOWNSTEP.Total Beneficiaries taken Lab tests..!") 
        # df_tc['Tests Conducted-Cumulative Count']=list(df_tc_init['Total Beneficiaries taken Lab tests'])
