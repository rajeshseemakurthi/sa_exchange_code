...
df_column_tbv = pd.DataFrame({'Location':pd.Series(dtype='str'),
                        'Total Beneficiary Visits Yesterday': pd.Series(dtype='int'),
                        'Total Beneficiary Visits-Cumulative Count': pd.Series(dtype='int')})
...
def layout():
    ...
    html.Div([
    html.Div(dcc.DatePickerRange(id='my_date_picker_range_tbv',display_format='D-M-Y',min_date_allowed=date(2022, 2, 3),max_date_allowed=obj_datepicker_tbv.current_date(),initial_visible_month=date(2022, 2, 3),start_date=date(2022, 2, 3),end_date=obj_datepicker_tbv.current_date(),style={"position":"relative","display":"inline-block","width":"110%","border-radius":"39px","margin-left":"115%","margin-top":"5%","margin-bottom":"7%"}),style={}),
    ...
    html.A([html.I(className="fa fa-refresh",style={})],href=dash.page_registry['pages.c_tbv']['path'],style={"width":"4%","height":"2.3rem","margin-left":"0.8rem","margin-top":"1.5%","text-align":"center","font-size":"22px","background-color":"rgb(215,215,215)","border-radius":"8px","padding-top":"0.3rem","border":"2px double black"})
    ...
    dash_table.DataTable(columns=[{'id': c, 'name': c} for c in df_column_tbv.columns],id="table_tbv",
                        ...
                        style_table={"width":"606px","margin-left":"18.5%","margin-bottom":"4%"},
...
@app.callback(Output('table_tbv','data'),Output('c_tbv_error','children'),
    inputs=dict(start_date=Input('my_date_picker_range_tbv','start_date'),end_date=Input('my_date_picker_range_tbv','end_date')))
def update_tbv_selected(start_date,end_date):
    ...
    if start>end:
        ...
        #         tbv_n1.append(0)
        #     df_tbv['Total Beneficiary Visits Yesterday']=tbv_n1
        ...
        #     df_tbv['Total Beneficiary Visits Yesterday']=df_tbv_main_impo[0]
        # else:
        #     print("I am in else loop-DOWNSTEP.MMU Opeartional Days..!") 
        # df_tbv['Total Beneficiary Visits-Cumulative Count']=list(df_tbv_initial['Total Beneficiaries Registered'])
