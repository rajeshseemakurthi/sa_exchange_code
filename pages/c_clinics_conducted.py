...
df_column_clinic = pd.DataFrame({'Location':pd.Series(dtype='str'),
                        'Clinic Operations Done Yesterday': pd.Series(dtype='int'),
                        'Clinic Operations-Cumulative Count': pd.Series(dtype='int')})
...
def layout():
    ...
    html.Div([
    html.Div(dcc.DatePickerRange(id='my_date_picker_range_clinic',display_format='D-M-Y',min_date_allowed=date(2022, 2, 3),max_date_allowed=obj_datepicker_cc.current_date(),initial_visible_month=date(2022, 2, 3),start_date=date(2022, 2, 3),end_date=obj_datepicker_cc.current_date(),style={"position":"relative","display":"inline-block","width":"110%","border-radius":"39px","margin-left":"115%","margin-top":"5%","margin-bottom":"7%"}),style={}),
    ...
    html.A([html.I(className="fa fa-refresh",style={})],href=dash.page_registry['pages.c_clinics_conducted']['path'],style={"width":"4%","height":"2.3rem","margin-left":"0.8rem","margin-top":"1.5%","text-align":"center","font-size":"22px","background-color":"rgb(215,215,215)","border-radius":"8px","padding-top":"0.3rem","border":"2px double black"})
...
@app.callback(Output('table_clinic','data'),Output('c_clinic_error','children'),
    inputs=dict(start_date=Input('my_date_picker_range_clinic','start_date'),end_date=Input('my_date_picker_range_clinic','end_date')))
def update_clinic_selected(start_date,end_date):
    ...
    if start>end:
        ...
        # df_clinic_initial=pd.DataFrame(df_clinic.groupby(['Location'])['Clinic Operational Days'].sum()).reset_index()
        # df_clinic_operations['Location']=df_clinic_initial['Location']
        # clinic_n1=[]
        # if (df_yesterday.empty)==True:
        #     for i in range(len(df_clinic_initial)):
        #         clinic_n1.append(0)
        #     df_clinic_operations['Clinic Operations Done Yesterday']=clinic_n1
        # elif (df_yesterday.empty)!=True:
        #     df_ye=df[df['Date']==yesterday]
        #     df_yest=pd.DataFrame(df_ye.groupby(['Location'])['Clinic Operational Days'].sum()).reset_index()
            
        #     mk1=list(df_clinic_initial['Location'])
        #     #for Clinic Cumulative Yesterday Count Column
        #     df_yeste_fe=df_yest.groupby('Location')['Clinic Operational Days'].sum()
        ...
        #     df_clinic_operations['Clinic Operations Done Yesterday']=df_clinic_main[0]

        #     df_clinic_operations['Clinic Operations Done Yesterday']=df_yest['Clinic Operational Days']
        # else:
        #     print("I am in else loop-DOWNSTEP.Clinic Opeartional Days..!") 
        # df_clinic_operations['Clinic Operations-Cumulative Count']=list(df_clinic_initial['Clinic Operational Days'])
